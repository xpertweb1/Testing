...
class Gateway extends WC_Payment_Gateway
{
    ...
    public function rate_sources_list_filter( $rate_sources )
    {
    ...
    {
        $rate_sources = apply_filters( 'epg_rate_sources_list', [] );
    ...
    public function admin_enqueue_scripts( $hook )
    {
        ...
        wp_localize_script( 'wooetherc20paymentgateway-admin-main', 'epg', [
            ...
            'gas_limit'                                           => esc_html( $this->get_setting_( 'gas_limit', '400000' ) ),
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields += array(
            ...
            'type'        => 'text',
            'description' => __( '<p>The blockchain used: mainnet, ropsten, rinkeby, bsctest, bsc. Use mainnet or bsc in production, and ropsten, rinkeby or bsctest in test mode.</p>', 'ether-and-erc20-tokens-woocommerce-payment-gateway' ),
            'default'     => 'mainnet',
        ),
            'gas_limit'            => array(
    ...
    )
    {
        ...
        if ( 'Jobster' == wp_get_theme()->name || 'Jobster' == wp_get_theme()->parent_theme ) {
            ...
            if ( !is_null( $eth_value ) ) {
                ...
                $eth_value_wei = round( doubleval( $eth_value ) * $decimals_eth, 5, PHP_ROUND_HALF_UP );
                // payment recieved
                foreach ( $paymentInfo as $currencyPayment => $valuePayment ) {
                    ...
                    if ( '0x0000000000000000000000000000000000000001' == $currencyPayment ) {
                        $paymentSuccess = round( $valuePayment, 5, PHP_ROUND_HALF_UP ) >= $eth_value_wei;
                    } else {
    ...
    protected function getPaymentInfo( $order_id, $providerUrl, $marketAddress )
    {
        ...
        if ( !($currencyPayment === "0x0000000000000000000000000000000000000000" || $currencyPayment === "0x") ) {
            ...
            if ( null !== $valuePayment ) {
                ...
                return [
                    $currencyPayment => $valuePayment_f,
                ];
            }
