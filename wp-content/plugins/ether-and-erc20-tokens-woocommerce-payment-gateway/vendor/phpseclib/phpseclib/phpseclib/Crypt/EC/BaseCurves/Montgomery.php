 ...
 */
namespace Ethereumico\Epg\Dependencies\phpseclib3\Crypt\EC\BaseCurves;
...
use Ethereumico\Epg\Dependencies\phpseclib3\Math\PrimeField\Integer as PrimeInteger;
/**
 * Curves over y^2 = x^3 + a*x + x
 *
 * @package EC
 ...
 * @access  public
 */
class Montgomery extends \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\EC\BaseCurves\Base
{
    /**
     ...
     * Sets the modulo
     */
    public function setModulo(\Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger $modulo)
    {
        $this->modulo = $modulo;
        $this->factory = new \Ethereumico\Epg\Dependencies\phpseclib3\Math\PrimeField($modulo);
        $this->zero = $this->factory->newInteger(new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger());
        $this->one = $this->factory->newInteger(new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger(1));
    }
    /**
     * Set coefficients a
     */
    public function setCoefficients(\Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger $a)
    {
        if (!isset($this->factory)) {
            throw new \RuntimeException('setModulo needs to be called before this method');
        }
        $this->a = $this->factory->newInteger($a);
        $two = $this->factory->newInteger(new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger(2));
        $four = $this->factory->newInteger(new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger(4));
    ...
    {
        switch (\true) {
            case !$x instanceof \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger && !$x instanceof \Ethereumico\Epg\Dependencies\phpseclib3\Math\PrimeField\Integer:
            ...
            case !$y instanceof \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger && !$y instanceof \Ethereumico\Epg\Dependencies\phpseclib3\Math\PrimeField\Integer:
        ...
        $this->p = [$x instanceof \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger ? $this->factory->newInteger($x) : $x, $y instanceof \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger ? $this->factory->newInteger($y) : $y];
    }
    /**
     ...
     * @return FiniteField[][]
     */
    private function doubleAndAddPoint(array $p, array $q, \Ethereumico\Epg\Dependencies\phpseclib3\Math\PrimeField\Integer $x1)
    {
        if (!isset($this->factory)) {
            throw new \RuntimeException('setModulo needs to be called before this method');
        }
        ...
        $x4 = $aa->multiply($bb);
        $temp = static::class == \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\EC\Curves\Curve25519::class ? $bb : $aa;
     ...
     * @return array
     */
    public function multiplyPoint(array $p, \Ethereumico\Epg\Dependencies\phpseclib3\Math\Common\FiniteField\Integer $d)
    {
