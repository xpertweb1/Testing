 ...
 */
namespace Ethereumico\Epg\Dependencies\phpseclib3\Math\PrimeField;

use Ethereumico\Epg\Dependencies\phpseclib3\Math\Common\FiniteField\Integer as Base;
 ...
 * @access  public
 */
class Integer extends \Ethereumico\Epg\Dependencies\phpseclib3\Math\Common\FiniteField\Integer
{
    /**
     ...
     * Default constructor
     */
    public function __construct($instanceID, \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger $num = null)
    {
     ...
     */
    public static function setModulo($instanceID, \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger $modulo)
    {
    ...
    public static function setRecurringModuloFunction($instanceID, callable $function)
    {
        ...
        if (!isset(static::$zero)) {
            static::$zero = new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger();
        }
    }
    /**
     * Returns the modulo
     ...
     * @return static
     */
    public function pow(\Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger $x)
    {
        $temp = new static($this->instanceID);
    ...
    public function squareRoot()
    {
        ...
        if (!isset($one)) {
            ...
            $two = new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger(2);
        }
        $reduce = static::$reduce[$this->instanceID];
        $p_1 = static::$modulo[$this->instanceID]->subtract($one);
        $q = clone $p_1;
        $s = \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger::scan1divide($q);
        ...
        $m = new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger($s);
    ...
    {
        return \Ethereumico\Epg\Dependencies\ParagonIE\ConstantTime\Hex::encode($this->toBytes());
    }
    /**
    ...
    {
        $w++;
        $mask = new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger((1 << $w) - 1);
        $sub = new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger(1 << $w);
