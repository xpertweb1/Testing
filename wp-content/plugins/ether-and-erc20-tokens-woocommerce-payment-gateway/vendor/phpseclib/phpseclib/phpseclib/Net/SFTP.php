...
use Ethereumico\Epg\Dependencies\phpseclib3\Exception\FileNotFoundException;
 ...
 * @access  public
 */
class SFTP extends \Ethereumico\Epg\Dependencies\phpseclib3\Net\SSH2
{
    /**
    ...
    public function __construct($host, $port = 22, $timeout = 10)
    {
        ...
        if (!\defined('Ethereumico\\Epg\\Dependencies\\NET_SFTP_QUEUE_SIZE')) {
            \define('Ethereumico\\Epg\\Dependencies\\NET_SFTP_QUEUE_SIZE', 32);
        }
        if (!\defined('Ethereumico\\Epg\\Dependencies\\NET_SFTP_UPLOAD_QUEUE_SIZE')) {
            \define('Ethereumico\\Epg\\Dependencies\\NET_SFTP_UPLOAD_QUEUE_SIZE', 1024);
        }
    }
    /**
     * Login
    ...
    private function init_sftp_connection()
    {
        ...
        $packet = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('CsN3', NET_SSH2_MSG_CHANNEL_OPEN, 'session', self::CHANNEL, $this->window_size, 0x4000);
        $this->send_binary_packet($packet);
        $this->channel_status[self::CHANNEL] = NET_SSH2_MSG_CHANNEL_OPEN;
        $this->get_channel_packet(self::CHANNEL, \true);
        $packet = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('CNsbs', NET_SSH2_MSG_CHANNEL_REQUEST, $this->server_channels[self::CHANNEL], 'subsystem', \true, 'sftp');
        $this->send_binary_packet($packet);
        ...
        if ($response === \false) {
            ...
            // is redundant
            $packet = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('CNsCs', NET_SSH2_MSG_CHANNEL_REQUEST, $this->server_channels[self::CHANNEL], 'exec', 1, $command);
            $this->send_binary_packet($packet);
            $this->channel_status[self::CHANNEL] = NET_SSH2_MSG_CHANNEL_REQUEST;
            $response = $this->get_channel_packet(self::CHANNEL, \true);
            if ($response === \false) {
        ...
        list($this->version) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('N', $response);
        while (!empty($response)) {
            list($key, $value) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('ss', $response);
    ...
    {
        if ($status == -1) {
            list($status) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('N', $response);
        }
    ...
    public function realpath($path)
    {
        ...
        if ($this->pwd === \false) {
            ...
            $this->send_sftp_packet(NET_SFTP_REALPATH, \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $path));
            $response = $this->get_sftp_packet();
            switch ($this->packet_type) {
                case NET_SFTP_NAME:
                    ...
                    list(, $filename) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('Ns', $response);
                    return $filename;
    ...
    {
        if (!($this->bitmap & \Ethereumico\Epg\Dependencies\phpseclib3\Net\SSH2::MASK_LOGIN)) {
            return \false;
        }
        ...
        $this->send_sftp_packet(NET_SFTP_OPENDIR, \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $dir));
    ...
    private function readlist($dir, $raw = \true)
    {
        ...
        while (\true) {
            ...
            $this->send_sftp_packet(NET_SFTP_READDIR, \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $handle));
            $response = $this->get_sftp_packet();
            switch ($this->packet_type) {
                case NET_SFTP_NAME:
                    list($count) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('N', $response);
                    for ($i = 0; $i < $count; $i++) {
                        list($shortname, $longname) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('ss', $response);
    ...
    private function stat_helper($filename, $type)
    {
        ...
        $packet = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $filename);
    ...
    public function touch($filename, $time = null, $atime = null)
    {
        ...
        $packet = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('sN', $filename, $flags) . $attr;
    ...
    private function setstat($filename, $attr, $recursive)
    {
        ...
        $this->send_sftp_packet(NET_SFTP_SETSTAT, \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $filename) . $attr);
        /*
    ...
    private function setstat_recursive($path, $attr, &$i)
    {
        ...
        foreach ($entries as $filename => $props) {
            ...
            if ($props['type'] == NET_SFTP_TYPE_DIRECTORY) {
                ...
                $this->send_sftp_packet(NET_SFTP_SETSTAT, \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $temp) . $attr);
                $i++;
        ...
        $this->send_sftp_packet(NET_SFTP_SETSTAT, \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $path) . $attr);
        $i++;
        if ($i >= NET_SFTP_QUEUE_SIZE) {
            if (!$this->read_put_responses($i)) {
                return \false;
    ...
    public function readlink($link)
    {
        ...
        $this->send_sftp_packet(NET_SFTP_READLINK, \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $link));
        $response = $this->get_sftp_packet();
        switch ($this->packet_type) {
            case NET_SFTP_NAME:
                break;
        ...
        list($filename) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $response);
        return $filename;
    }
    /**
     * Create a symlink
    ...
    public function symlink($target, $link)
    {
        ...
        $link = $this->realpath($link);
        $packet = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('ss', $target, $link);
    ...
    private function mkdir_helper($dir, $mode)
    {
        ...
        $this->send_sftp_packet(NET_SFTP_MKDIR, \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $dir) . "\0\0\0\0");
        $response = $this->get_sftp_packet();
        if ($this->packet_type != NET_SFTP_STATUS) {
            throw new \UnexpectedValueException('Expected NET_SFTP_STATUS. ' . 'Got packet type: ' . $this->packet_type);
        }
    ...
    public function rmdir($dir)
    {
        ...
        $this->send_sftp_packet(NET_SFTP_RMDIR, \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $dir));
        $response = $this->get_sftp_packet();
        if ($this->packet_type != NET_SFTP_STATUS) {
            throw new \UnexpectedValueException('Expected NET_SFTP_STATUS. ' . 'Got packet type: ' . $this->packet_type);
        }
    ...
    public function put($remote_file, $data, $mode = self::SOURCE_STRING, $start = -1, $local_start = -1, $progressCallback = null)
    {
        ...
        $packet = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('sNN', $remote_file, $flags, 0);
        $this->send_sftp_packet(NET_SFTP_OPEN, $packet);
        $response = $this->get_sftp_packet();
        switch ($this->packet_type) {
            case NET_SFTP_HANDLE:
        ...
        switch (\true) {
            ...
            case $mode & self::SOURCE_LOCAL_FILE:
                if (!\is_file($data)) {
                    throw new \Ethereumico\Epg\Dependencies\phpseclib3\Exception\FileNotFoundException("{$data} is not a valid file");
                }
                $fp = @\fopen($data, 'rb');
                if (!$fp) {
                    return \false;
    ...
    public function get($remote_file, $local_file = \false, $offset = 0, $length = -1, $progressCallback = null)
    {
        ...
        while (\true) {
            ...
            while ($i < NET_SFTP_QUEUE_SIZE && ($length < 0 || $read < $length)) {
                ...
                $packet = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('sN3', $handle, $tempoffset / 4294967296, $tempoffset, $packet_size);
                try {
    ...
    private function delete_recursive($path, &$i)
    {
        ...
        foreach ($entries as $filename => $props) {
            ...
            if ($props['type'] == NET_SFTP_TYPE_DIRECTORY) {
                ...
                $this->send_sftp_packet(NET_SFTP_REMOVE, \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $temp));
        ...
        $this->send_sftp_packet(NET_SFTP_RMDIR, \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $path));
        $this->remove_from_stat_cache($path);
        $i++;
        if ($i >= NET_SFTP_QUEUE_SIZE) {
            if (!$this->read_put_responses($i)) {
    ...
    {
        $packet = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('sNN', $this->realpath($path), NET_SFTP_OPEN_READ, 0);
        $this->send_sftp_packet(NET_SFTP_OPEN, $packet);
        $response = $this->get_sftp_packet();
        switch ($this->packet_type) {
            case NET_SFTP_HANDLE:
    ...
    {
        $packet = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('sNN', $this->realpath($path), NET_SFTP_OPEN_WRITE, 0);
        $this->send_sftp_packet(NET_SFTP_OPEN, $packet);
        $response = $this->get_sftp_packet();
        switch ($this->packet_type) {
            case NET_SFTP_HANDLE:
    ...
    public function rename($oldname, $newname)
    {
        ...
        // http://tools.ietf.org/html/draft-ietf-secsh-filexfer-13#section-8.3
        $packet = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('ss', $oldname, $newname);
    ...
    {
        $attr = [];
        list($flags) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('N', $response);
        ...
        foreach ($this->attributes as $key => $value) {
            switch ($flags & $key) {
                case NET_SFTP_ATTR_SIZE:
                    ...
                    list($upper, $size) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('NN', $response);
                    $attr['size'] = $upper ? 4294967296 * $upper : 0;
                    $attr['size'] += $size < 0 ? ($size & 0x7fffffff) + 0x80000000 : $size;
                    break;
                case NET_SFTP_ATTR_UIDGID:
                    // 0x00000002 (SFTPv3 only)
                    list($attr['uid'], $attr['gid']) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('NN', $response);
                    break;
                case NET_SFTP_ATTR_PERMISSIONS:
                    // 0x00000004
                    list($attr['mode']) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('N', $response);
                ...
                case NET_SFTP_ATTR_ACCESSTIME:
                    // 0x00000008
                    list($attr['atime'], $attr['mtime']) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('NN', $response);
                    break;
    ...
    private function send_sftp_packet($type, $data, $request_id = 1)
    {
        ...
        $stop = \microtime(\true);
        if (\defined('Ethereumico\\Epg\\Dependencies\\NET_SFTP_LOGGING')) {
    ...
    private function get_sftp_packet($request_id = null)
    {
        ...
        \extract(\unpack('Nlength', \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::shift($this->packet_buffer, 4)));
        /** @var integer $length */
        $tempLength = $length;
        ...
        $stop = \microtime(\true);
        $this->packet_type = \ord(\Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::shift($this->packet_buffer));
        if ($this->use_request_id) {
            \extract(\unpack('Npacket_id', \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::shift($this->packet_buffer, 4)));
        ...
        $packet = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::shift($this->packet_buffer, $length);
        if (\defined('Ethereumico\\Epg\\Dependencies\\NET_SFTP_LOGGING')) {
