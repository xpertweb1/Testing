 ...
 * @license MIT
 */
namespace Ethereumico\Epg\Dependencies\Web3\Methods;

use InvalidArgumentException;
use RuntimeException;
use Ethereumico\Epg\Dependencies\Web3\Methods\IMethod;
use Ethereumico\Epg\Dependencies\Web3\Methods\JSONRPC;
class EthMethod extends \Ethereumico\Epg\Dependencies\Web3\Methods\JSONRPC implements \Ethereumico\Epg\Dependencies\Web3\Methods\IMethod
{
    /**
     * validators
     * 
            ...
            if (isset($params[$key])) {
                if (\is_array($rule)) {
                    ...
                    foreach ($rule as $subRule) {
                        if (\call_user_func([$subRule, 'validate'], $params[$key]) === \true) {
                            $isError = \false;
                            break;
                        }
                    }
