...
class CallValidator
{
    ...
    public static function validate($value)
    {
        ...
        if (isset($value['from']) && \Ethereumico\Epg\Dependencies\Web3\Validators\AddressValidator::validate($value['from']) === \false) {
            return \false;
        }
        if (!isset($value['to'])) {
            return \false;
        }
        if (\Ethereumico\Epg\Dependencies\Web3\Validators\AddressValidator::validate($value['to']) === \false) {
            return \false;
        }
