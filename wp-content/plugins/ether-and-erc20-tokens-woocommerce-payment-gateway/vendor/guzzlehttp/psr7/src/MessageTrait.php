...
trait MessageTrait
{
    ...
    {
        if (!$this->stream) {
            $this->stream = \Ethereumico\Epg\Dependencies\GuzzleHttp\Psr7\Utils::streamFor('');
        }
        return $this->stream;
    }
    public function withBody(\Ethereumico\Epg\Dependencies\Psr\Http\Message\StreamInterface $body)
    {
        if ($body === $this->stream) {
            return $this;
        }
