...
function tax_icons_add_taxonomy_filters() {
	...
	foreach( get_taxonomies() as $taxonomy ) {
		...
		add_action( $taxonomy . '_add_form_fields', 'enable_global_add_meta_field' );

		// Edit taxonomy term
		add_action( $taxonomy . '_edit_form_fields', 'enable_global_edit_meta_field' );
		...
		add_action( 'edited_' . $taxonomy, 'enable_global_save_taxonomy_custom_meta', 10, 2 );
		add_action( 'create_' . $taxonomy, 'enable_global_save_taxonomy_custom_meta', 10, 2 );
...
function enable_global_add_meta_field() {
	// this will add the custom meta field to the add new term page
	?>
	<div class="form-field">
		<lable>Enable this Marktype Globally</lable>
		<p><input type="checkbox" name="term_meta[enable_marktype_global]" value="on"></p>
	</div>
	<?php
}
...
function enable_global_edit_meta_field( $term ) {

	// Get the term ID
	$term_id = $term->term_id;

	// Retrieve the existing value(s) for this meta field (array)
	$term_meta = get_option( 'enable_marktype_global_' . $term_id );
	$term_meta=$term_meta['enable_marktype_global'];


	//print_r($term_meta['enable_marktype_global']);
	//print_r($term);
	...
	<tr class="form-field">
	<th scope="row" valign="top">		<lable>Enable this Marktype Globally</lable>
</th>
		<td>
			<input type="checkbox" name="term_meta[enable_marktype_global]" value="on" <?php  if(!empty($term_meta) && $term_meta=='on'){ echo 'checked'; } ?>>
		</td>
	</tr>
	<?php
}
...
function enable_globl_edit_meta_field( $term ) {
...
function enable_global_save_taxonomy_custom_meta( $term_id ) {
	

	if ( isset( $_POST['term_meta'] ) ) {


		$term_meta=array('enable_marktype_global' => $_POST['term_meta']['enable_marktype_global']);
		update_option( 'enable_marktype_global_'.$term_id , $term_meta );
	}
